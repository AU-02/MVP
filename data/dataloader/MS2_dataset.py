...
class DataLoader_MS2(data.Dataset):
    ...
    def __init__(self, root, seed=None, data_split='train', modality='thr', \
        ...
        if 'txt' in data_list_file:                
            ...
            self.seq_list = [seq_name.strip() for seq_name in open(data_list_file)]
        ...
        if data_format == 'MonoDepth': # Monocular depth estimation, dict: {'img', 'depth'}
            ...
            # Check if proj_depth directory exists to determine sync mode
            proj_depth_path = self.root / 'proj_depth'
            if proj_depth_path.exists():
                self.crawl_folders_depth(sampling_step, set_length, set_interval)
            else:
                print("[INFO] No proj_depth found. Using sync mode without depth.")
                self.crawl_folders_sync()
    ...
    def set_extrinsics(self) :
        ...
        print(f"[DEBUG] calib_path trying to load: {calib_path}")
    ...
    def crawl_folders_depth(self, sampling_step, set_length, set_interval):
        ...
        for sensor in sensor_list:               
            ...
            for seq in self.seq_list: # iterate over each sequence
                ...
                # crawl the data path
                min_len = min(len(img_list_left), len(img_list_right))
                img_list_left = img_list_left[:min_len]
                img_list_right = img_list_right[:min_len]
            ...
            sample_set = sample_set[0::sampling_step]
    ...
    def crawl_folders_sync(self):
        """Crawl folders for sync_data without proj_depth."""
        self.samples = []

        # Check available folders
        self.has_nir = osp.exists(osp.join(self.root, "nir"))
        self.has_rgb = osp.exists(osp.join(self.root, "rgb"))
        self.has_imu = osp.exists(osp.join(self.root, "gps_imu"))

        seqs = sorted(os.listdir(osp.join(self.root, "rgb")))  # assuming all modalities share sequence structure

        for seq in seqs:
            rgb_dir = osp.join(self.root, "rgb", seq, "image_0")
            nir_dir = osp.join(self.root, "nir", seq, "image_0") if self.has_nir else None

            if not osp.isdir(rgb_dir):
                continue

            rgb_images = sorted(glob.glob(osp.join(rgb_dir, "*.png")))
            nir_images = sorted(glob.glob(osp.join(nir_dir, "*.png"))) if nir_dir else [None] * len(rgb_images)

            for rgb_path, nir_path in zip(rgb_images, nir_images):
                sample = {
                    "rgb": rgb_path,
                    "nir": nir_path,
                    "depth": None,
    ...
    def get_data_MonoDepth(self, index, modality):
        ...
        if modality == 'nir':
            print(f"[INFO] Loading NIR image: {sample['tgt_img_left']}")
        ...
        result["tgt_depth_gt_path"] = str(sample['tgt_depth_gt'])
        return result
    ...
    def get_data_Odometry(self, index, modality):
        ...
        result["pose"]     = torch.as_tensor(pose) # Absolute pose (w.r.t first image of the sequence)
        return result